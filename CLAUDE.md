...
- **xCard component interaction protocol** — three classes: (A) action-only: `Button`/`ModalButton` call `onAction(name, {})`, never pass context directly; (B) dataModel-only: all form inputs (`Input`/`Slider`/`RadioGroup` etc.) call `onDataChange(path, value)` only, A-class actions read values via path refs; (C) hybrid: `Select` calls both `onDataChange` and `onAction(name, {...context, value: v})` — the explicit `value` in context lets the handler receive the selection synchronously without path resolution. Full protocol documented in `srcjs/xCardDefaults.tsx` header comments. **Timing study (examples/test_selectize_timing.R)**: "select-then-submit" pattern (Class B onDataChange on select + Class A onAction on button click) DOES work — path refs resolve correctly at button-click time when dataModel is already stable. Only "select-immediately-triggers-action" patterns (data_only, micro/macro delayed) fail. This means: if a future component separates selection from action submission, it can use pure Class B + a submit button and drop hybrid entirely. **Protocol red lines** (see `docs/xcard-interaction-protocol.md` for full spec): (1) never spread `action.event.context` in `onAction` — it blocks path resolution; (2) Select hybrid is not a template for other components; (3) new input components default to Class B; (4) any "select-immediately-triggers-action" request for Tabs/Segmented/RadioGroup/ChoicePicker requires a timing test first — expected to need hybrid or submit_action.
- **xCard boundary value handling** — `InputNumber` and `DateTimeInput` write `null` to dataModel on mount when no initial value, so R handler receives `NULL` (not missing). `CheckBox` initial value coerced via `checked === true || checked === 1` — guards against upstream passing string `"false"` which `!!` would wrongly coerce to `true`.
