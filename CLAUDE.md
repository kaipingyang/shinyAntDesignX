...
- **Three notification widgets, three layers** — (1) `notification` widget wraps `@ant-design/x` `XNotification` = OS/system-level browser push (needs OS + browser permission, shows outside the page, for AI-agent long-task alerts). (2) `message` widget wraps antd `message` = in-page top-center single-line toast (no permission). (3) `notify` widget wraps antd `notification` = in-page corner card with title+description (no permission). message/notify use the **hook API** (`message.useMessage()` / `notification.useNotification()`) not static methods, so they respect ConfigProvider theme. All three are hidden outputs (height 0) — repeat fires require `outputOptions(suspendWhenHidden = FALSE)` or bumping a `ts` field. message/notify need only antd deps (no antdx).
...
| Notify (antd in-page) | `antDesignNotificationOutput` | `renderAntDesignNotification` |
