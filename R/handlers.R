...
#' Create an ellmer streaming handler for antDesignXServer
#'
#' Wraps an `ellmer` chat object into an `antDesignXServer`-compatible handler.
#' Supports per-thread conversation history, tool calling with optional
...
#'
#' @param store A session store from [ellmer_session_store()].
...
#' Create a ClaudeAgentSDK handler for antDesignXServer
#'
#' Wraps `ClaudeAgentSDK` into an `antDesignXServer`-compatible handler.
...
#'
#' @param directory Project directory to filter sessions.
...
#'
#' @param session_map_path Path to the session map `.rds` file.
#' @return A function with signature `function(session_id, thread_id, send_thread)`.
#' @export
